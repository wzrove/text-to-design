...
| [0016](0016-i18n走语义键加参数，默认语言取系统语言并可手动切换.md) | i18n 走语义键加参数，默认语言取系统语言并可手动切换 | 已采纳（大部分落地：B0+B2+B3 键化+B4 机制已落；en 译文待补，棘轮守） | `shared/`（新增 `dicts/i18n/`、`locale-channel.ts`、`core/host.ts` 的 clientStorage、`dicts/platform|capability.ts` label 投影、`schemas/*` describe、`core/*` 错误文案）、`ui/`（新增 `src/i18n/*`、`components/LocaleSwitch.tsx`、`App.tsx`、`components/*`、`bridge/*`、`code/plugin.ts`、`ui.html`）、`mcp-server/`（`i18n.ts` 新、`core/localize-schema.ts` 新、`tools/*`、`server.ts`、`core/registry.ts`）、`tests/i18n.test.ts` | 2026-09-21 |

...
常用取值路径速查(路径与各工具返回结构对应,写错会报「无法解析占位符引用」):created 为数组 → {{id.created[0].id}}(per-type create 工具 jsd_create_rectangle/jsd_create_frame 等,以及 jsd_clone_node / jsd_create_instance / jsd_outline_stroke);created 为单对象 → {{id.created.id}}(jsd_create_svg / jsd_create_icon / jsd_group_nodes / jsd_flatten_nodes / jsd_create_component / jsd_import_component / jsd_combine_as_variants);jsd_find → {{id.nodes[0].id}};jsd_set_* → {{id.updated[0].id}};jsd_swap_component → {{id.swapped[0].id}}。`,

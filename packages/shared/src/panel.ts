...
/**
 * 日志抽屉打开期间的窗口高度下限(取历史默认高度,见 0015)。
 *
 * 抽屉自身取窗口的 85%,而自适应布局下窗口高度 = 内容高度 —— 内容可以短到
 * `PANEL_HEIGHT_MIN`(300),那会把抽屉压成 255px 的矮条:扣掉它自己的头部与过滤行,
 * 只剩六行日志。日志是「浏览」不是「瞥一眼」,它的可用高度不该由无关内容的多少决定,
 * 故打开期间把窗口抬到这个下限。开合时窗口跳一次是有意接受的成本:它是用户显式动作,
 * 且宽度固定 360、不存在重排回环。
 */
export const PANEL_HEIGHT_LOG = PANEL_HEIGHT_DEFAULT;

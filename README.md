...
4. 上述三步完成后,告知用户剩下两步需手动:
   - 在即时设计里「插件 → 导入」,选择 ~/text-to-design-plugin/dist/jsdesign/manifest.json
     (Figma 则导入 dist/figma/manifest.json;MasterGo 用「插件 → 开发者模式 → 创建/添加插件」
     上传 dist/mastergo/manifest.json),
     运行插件,面板显示「已连接」即就绪
...
打开即时设计客户端,点「插件」→「导入」,选择 `~/text-to-design-plugin/dist/jsdesign/manifest.json`,然后回到画布运行这个插件。面板上显示「已连接」,就说明准备好了。(Figma 用户:菜单「Plugins → Development → Import plugin from manifest」导入 `dist/figma/manifest.json`;MasterGo 用户:「插件 → 开发者模式 → 创建/添加插件」上传 `dist/mastergo/manifest.json`。)
...
pnpm build      # 构建插件包(packages/ui/dist/{jsdesign,figma,mastergo}/)
